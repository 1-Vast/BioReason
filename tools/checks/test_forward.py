...
sys_path.insert(0, str(__import__("pathlib").Path(__file__).resolve().parents[2]))
...
# 1) No evidence_dim 鈫?no evidence allowed
...
print("  1) no evidence_dim, no evidence 鈫?OK")

# 2) evidence_dim=ED 鈫?evidence works
...
print(f"  2) evidence_dim={ED} 鈫?evidence_pred {tuple(out2['evidence_pred'].shape)} OK")

# 3) evidence=None 鈫?evidence_pred is None
out3 = m2(x, pert, evidence=None)
assert out3["evidence_pred"] is None
print("  3) evidence=None 鈫?evidence_pred is None OK")

# 4) evidence_dim not set but evidence given 鈫?error
try:
    m1(x, pert, evidence=ev)
    assert False, "should have raised"
except ValueError as e:
    assert "evidence_dim" in str(e)
    print("  4) evidence_dim missing 鈫?ValueError OK")
